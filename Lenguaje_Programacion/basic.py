...
class Lexer:
		...
		#metodo que avanzara al siguiente caracter
		def advance(self):
...
class Number:
	...
	def set_pos(self, pos_start = None, pos_end = None): #funcion para saber la posicion en caso de que llegue a existir una division por cero se detecta la posicion del error 
		self.pos_start = pos_start
		self.pos_end = pos_end
		return self
	...
	## metodos para operar los numeros
	#adicion
	def added_to(self, other):
		if isinstance(other, Number): #verificar que el dato recibido es un numero
...
class Interpreter:
	...
	def visit_BinOpNode(self, node, context):
		res = RTResult()
		left = res.registrer(self.visit(node.left_node, context))
		if res.error: return res
		right= res.registrer(self.visit(node.right_node, context))
		if res.error: return res
		
		if node.op_tok.type ==TT_SUMA:
			result, error = left.added_to(right)
			
		elif node.op_tok.type ==TT_RESTA:
	...
	def visit_UnaryOpNode(self, node, context):
		res = RTResult()	
		number = res.registrer(self.visit(node.node, context))
		if res.error: return res
		error = None
		if node.op_tok.type == TT_RESTA:
