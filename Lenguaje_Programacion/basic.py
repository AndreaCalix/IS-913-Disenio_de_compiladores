...
#sentencia For 
class ForNode:
	def __init__(self,var_name_tok, star_value_node, end_value_node, step_value_node, body_node):
		self.var_name_tok = var_name_tok
		self.start_value_node = star_value_node
...
############### sentencia IF
	...
	def for_expr(self):
		...
		if not self.current_tok.matches(TT_LLAVE, 'FOR'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Esperaba un  'FOR'"
			))
		...
		if not self.current_tok.matches(TT_LLAVE, 'TO'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Esperaba 'TO'"
			))
		...
		if self.current_tok.matches(TT_LLAVE, 'STEP'):
			res.register_advancement()
			self.advance()
	...
	def while_expr(self):
		...
		if not self.current_tok.matches(TT_LLAVE, 'WHILE'):
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				f"Esperaba 'WHILE'"
			))
...
#estan segun la gramatica utilizada en el lenguaje
	def atom(self):
        ...
        #para for 
		elif tok.matches(TT_LLAVE, 'FOR'):
			for_expr = res.register(self.for_expr())
			if res.error: return res
			return res.success(for_expr)
        #para while
		elif tok.matches(TT_LLAVE, 'WHILE'):
